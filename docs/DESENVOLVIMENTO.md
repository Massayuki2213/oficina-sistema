# Hermes — Ambiente de Desenvolvimento

Documentação técnica do monorepo. Para a visão de produto, veja o [README](../README.md).
Arquitetura, regras de negócio e roadmap: [PLANEJAMENTO.md](../PLANEJAMENTO.md).

## Estrutura

```
oficina-sistema/
├── PLANEJAMENTO.md         # documento de planejamento e arquitetura (v1.2)
...
`agenda` · `auth` · `backup` · `caixa` · `carros` · `clientes` · `compras` · `contas` ·
`despesas` · `fornecedores` · `health` · `orcamentos` · `ordens` · `pecas` ·
`relatorios` · `servicos` · `usuarios`

### Telas (`apps/desktop/src/pages/`)

`Dashboard` · `Clientes` · `Carros` · `Servicos` · `Estoque` · `Orcamentos` · `Ordens` ·
`Agenda` · `Caixa` · `Despesas` · `Contas` · `Compras` · `Distribuidores` · `Relatorios` ·
`Configuracoes` · `Login`
...
| `npm run infra:logs`  | Acompanha os logs dos containers                |
...
## Roadmap (seção 11 do PLANEJAMENTO)

- [x] **Fase 1 — Protótipo clicável** (`/prototipo`)
- [x] **Fase 2–4 — Base + fluxo completo** (cadastros, orçamento → OS, estoque, financeiro, agenda, login)
- [x] **Fase 5 — Usabilidade essencial** (editar/excluir, ficha do cliente, PDF, backup, máscaras)
- [ ] **Fase 6 — Produção e administração** ← *aqui* (Electron, configurações, alertas ativos, auditoria, testes)
- [ ] **Fase 7 — Diferenciais e fiscal** (WhatsApp, garantia, comissão, NF-e)
